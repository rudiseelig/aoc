...
input.split('\n').map((line) => {
  ...
  // magic
  const letterAtFirstPosition = password.charAt(first - 1) === letter
  const letterAtSecondPosition = password.charAt(second - 1) === letter
  if ((letterAtFirstPosition && !letterAtSecondPosition) || (!letterAtFirstPosition && letterAtSecondPosition)) {
    console.log(`${line} is correct`)
    count++
  }
})
console.log(count)
